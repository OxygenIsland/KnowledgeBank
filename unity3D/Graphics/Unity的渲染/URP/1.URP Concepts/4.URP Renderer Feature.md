...
tags:
---
Renderer Feature是一种资源，是插在渲染管线指定节点的“自定义渲染插件”。Unity 只负责把 Feature 挂在正确的时机（不透明前、透明后、后处理前等），具体干什么由你写：描边、SSAO、平面反射、额外 RT 拷贝、自定义阴影等都可以。Feature 之间互相独立，可随开随关，不影响主代码。

这里需要与另一个更常见的概念进行区分，那就是[[Command Buffer]]，它都是一个包含了渲染命令（如 DrawMesh, Blit, SetRenderTarget  等）的列表。它是一个纯粹的“数据容器”，告诉 GPU 需要执行哪些操作。但是它本身并不知道自己应该在渲染的哪个阶段被执行。
...
在 URP 中，你几乎不会脱离 Renderer Feature 单独使用 [[Command Buffer]]。实际流程如下：
