...
tags:
---
Universal Render Pipeline Asset是整条渲染管线的“总配置入口”，挂在 Graphics Settings 的 URP Asset 中。所有全局级别的开关（是否启用阴影、后处理、深度纹理、Native RenderPass、MSAA 级别等）都写在这里。可以理解为“我要什么效果”。我们可以拥有多个 URP 资源并在它们之间进行切换。例如，可以有一个启用阴影的资源和一个禁用阴影的资源。如果要在资源之间切换以查看效果，不必每次都手动切换相应的阴影设置。

要使用通用渲染管线 (Universal Render Pipeline, URP)，必须创建 URP Asset并在 Graphics settings 中分配该Asset，Unity 会从内置渲染管线切换到 URP。
