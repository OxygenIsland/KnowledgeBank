...
## 通用渲染器资源引用
Renderer是[[2.Universal Render Pipeline Asset（通用渲染管线资源）|Render Pipeline Asset]] 指向的“执行方案”，真正定义“怎么做”。里面记录了渲染路径（Forward/Deferred）、深度拷贝时机、Post-processing 数据、Stencil 覆盖以及最重要的——[[4.URP Renderer Feature|Renderer Features]] 列表。它把 Asset 里的“需求”翻译成具体的渲染 Pass 序列。
